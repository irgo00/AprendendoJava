...
public class Parser {
    ...
    public ArrayList<Plataforma> obterPlataformas(){
        ...
        ArrayList<String> plataformasString = new ArrayList<>();
        for (String linha: dados){
            String p = linha.split(separadorCSV)[3];
            if (!plataformasString.contains(p)){
                plataformasString.add(p);
            }            
        }
        ArrayList<Plataforma> plataforma = new ArrayList<>();
        for (String p: plataformasString){
    ...
    public ArrayList<Loja> obterLojas(){
        ...
        ArrayList<String> lojasString = new ArrayList<>();
        for (String linha: dados){
            String l = linha.split(separadorCSV)[2];
            if (!lojasString.contains(l)){
                lojasString.add(l);
            }            
        }
        ArrayList<Loja> lojas = new ArrayList<>();
        for (String l: lojasString){
    ...
    public ArrayList<Produto> obterProdutos(){
        ...
        for (int i = 0; i < nomeProdutoString.size(); i++){
            produtos.add(new Produto(nomeProdutoString.get(i), 
                    Double.parseDouble(precoProdutoString.get(i))));
        }       
        return produtos;
    }
    ...
    public ArrayList<Venda> obterVendas(){
        ...
        for (int i = 0; i < dados.size(); i++){
            ...
            Venda v = new Venda(pl, 
                                dd[0],
                                Integer.parseInt(dd[1]),
                                Integer.parseInt(dd[5]),
                                pd.getPrecoUnitario()*Integer.parseInt(dd[5]), lj, pd);
            vendas.add(v);
